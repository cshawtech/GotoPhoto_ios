...
protocol ARTrackingManagerDelegate : NSObjectProtocol
{
    ...
    func arTrackingManager(_ trackingManager: ARTrackingManager, didUpdateHeading: Double)
...
{    
    ...
    public var reportPeriodSeconds: Int = 5
    ...
    {
        self.stopReportLocationTimer()
      self.reportLocationTimer = Timer.scheduledTimer(timeInterval: TimeInterval(reportPeriodSeconds), target: self, selector: #selector(ARTrackingManager.reportLocationToDelegate), userInfo: nil, repeats: false)
    }
    ...
    internal func filterHeading()
    {
        ...
        self.filteredHeading = normalizeDegree(self.filteredHeading)
        self.delegate?.arTrackingManager(self, didUpdateHeading: self.filteredHeading)
    }
