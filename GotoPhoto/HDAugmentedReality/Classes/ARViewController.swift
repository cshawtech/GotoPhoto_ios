...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
    ...
    */
    ...
    var cameraView: CameraView = CameraView()
    ...
    fileprivate var guideLabel: UILabel?
    ...
    open func reload(reloadType currentReload: ARViewController.ReloadType)
    {
        ...
        if highestRankingReload == .annotationsChanged || highestRankingReload == .reloadLocationChanged || highestRankingReload == .userLocationChanged
        {
            ...
            self.showDirectionGuide()
        }
    
        self.presenter.reload(annotations: self.annotations, reloadType: highestRankingReload)
    }
  
    open func showDirectionGuide()
    {
      if let annotation = self.annotations.first
      {
        let heading = self.trackingManager.filteredHeading
        let bearing = annotation.azimuth
        var leftTurn = heading - bearing
        if leftTurn < 0
        {
          leftTurn += 360
        }
        if (leftTurn < 2) || (leftTurn > 358)
        {
          guideLabel?.text = "-----"
        }
        else if leftTurn < 180
        {
          guideLabel?.text = "<----"
        }
        else
    ...
    internal func arTrackingManager(_ trackingManager: ARTrackingManager, didUpdateHeading: Double)
    {
      showDirectionGuide()
    }
    ...
    func addDebugUi()
    {
        ...
        guideLabel = UILabel()
        guideLabel!.backgroundColor = UIColor.clear
        guideLabel!.textColor = UIColor.white
        guideLabel!.font = UIFont.boldSystemFont(ofSize: 50)
        guideLabel!.frame = CGRect(x: 5, y: 5, width: self.view.bounds.size.width - 10, height: 60)
        guideLabel!.numberOfLines = 0
        guideLabel!.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin]
        guideLabel!.textAlignment = NSTextAlignment.center
